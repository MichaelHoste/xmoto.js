...
(function() {
  var Assets, Blocks, Buffer, Constants, Edges, EngineSound, Entities, Ghost, Infos, Input, LayerOffsets, Level, Limits, Listeners, Math2D, Moto, MotoFlip, Physics, Replay, ReplayFile, Rider, Script, Sky, Theme, b2AABB, b2Body, b2BodyDef, b2CircleShape, b2DebugDraw, b2EdgeChainDef, b2EdgeShape, b2Fixture, b2FixtureDef, b2MassData, b2MouseJointDef, b2PolygonShape, b2PrismaticJointDef, b2RevoluteJointDef, b2Settings, b2Vec2, b2World, block_AABB, edge_AABB, entity_AABB, entity_texture_name, frame_name, full_screen, hide_loading, play_level, position_2d, show_loading, sort_blocks_by_texture, visible_block, visible_edge, visible_entity,
  ...
  Level = (function() {
    ...
    Level.prototype.load_as_replay = function(replay_name) {
      this.replay_name = replay_name;
      return this.load_replay();
    };

    Level.prototype.load_replay = function() {
      var replayFile;
      replayFile = new ReplayFile();
      replayFile.load_from_file(this.replay_name);
      return this.replayPlayer = new Ghost(this, replayFile.clone(this), "player");
    };
  ...
  ReplayFile = (function() {
    function ReplayFile() {
      this.load_replay = __bind(this.load_replay, this);
      this.frames = [];
    }

    ReplayFile.prototype.load_from_file = function(file_name) {
      return $.ajax({
        type: "GET",
        ...
        success: this.load_replay,
        async: false,
        context: this
      });
    };

    ReplayFile.prototype.clone = function(level) {
      var frame, new_replay, _i, _len, _ref;
    ...
    ReplayFile.prototype.map8BitsToCoord = function(ref, maxDiff, c) {
      return ref + c / 127 * maxDiff;
    };
    ...
    ReplayFile.prototype.load_replay = function(bin) {
      var format, replay;
    ...
    ReplayFile.prototype.load_replay_01 = function(replay, format) {
      var c, compressed, data, dataSize, endian_check, events, frameRate, n, nchunks, nstates, stateSize, zdata, zdataSize, _i, _results;
      endian_check = replay.getInt32();
      n = replay.getInt8();
      this.levelId = replay.getString(n);
      n = replay.getInt8();
      this.playerName = replay.getString(n);
      frameRate = replay.getFloat32();
      stateSize = replay.getInt32();
      this.finished = replay.getInt8() === 1;
      this.finishTime = replay.getFloat32();
      if (format === 1) {
      ...
      _results = [];
      for (c = _i = 0; 0 <= nchunks ? _i < nchunks : _i > nchunks; c = 0 <= nchunks ? ++_i : --_i) {
        ...
        _results.push(this.load_chunks(data, nstates));
      }
      return _results;
    };

    ReplayFile.prototype.load_chunks = function(data, nstates) {
      var backdiff, backdiff_norm, cBikeEngineRPM, centerPx, centerPy, chunk, elbowX, elbowY, fFrameRot, fFrontWheelRot, fRearWheelRot, flags, frame, frameX, frameY, frontWheelPx, frontWheelPy, gameTime, kneeX, kneeY, lowerBodyX, lowerBodyY, maxXDiff, maxYDiff, rearWheelPx, rearWheelPy, s, sensmult, shoulderX, shoulderY, unused, _i, _results;
      ...
      for (s = _i = 0; 0 <= nstates ? _i < nstates : _i > nstates; s = 0 <= nstates ? ++_i : --_i) {
        ...
        _results.push(this.frames.push(frame));
      }
      return _results;
    };
    ...
    ReplayFile.prototype.load_replay_3 = function(replay) {
      var todo;
      return todo = true;
    };

    ReplayFile.prototype.frames_count = function() {
      return this.frames.length;
    };

    ReplayFile.prototype.frame = function(number) {
      return this.frames[number];
    };
  ...
  Math2D = (function() {
    ...
    Math2D.plength = function(point) {
      return Math.sqrt(point.x * point.x + point.y * point.y);
    };

    Math2D.normalize = function(point) {
      var new_point, v;
      v = this.plength(point);
      if (v === 0) {
        return 0;
      }
      return new_point = {
