...
(function() {
  ...
  Physics = (function() {
    ...
    class Physics {
      ...
      create_polygons_collisions(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        var body, convex_polygon, l, len, len1, m, polygon, ref, ref1, ref2, ref3, ref4, ref5, ref6, ref7, ref8, shape, sub_polygon;
      ...
      create_rectangles_collisions(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        var body, dx, dy, i, l, len, length, offsetX, offsetY, polygon, px, py, ref, ref1, ref2, ref3, ref4, ref5, ref6, shape, v1, v2, vertex;
        polygon = new Polygon(vertices);
        polygon.optimize(); // remove duplicate/collinear
        if (polygon.length() < 3) {
      ...
      create_edges_collisions(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        var body, i, l, len, polygon, ref, ref1, ref2, ref3, ref4, ref5, ref6, shape, vertex, vertex1, vertex2;
        polygon = new Polygon(vertices);
        polygon.optimize(); // remove duplicate/collinear
        if (polygon.length() < 3) {
      ...
      create_chains_collisions(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        var body, chain, chains, l, len, polygon, ref, ref1, ref2, ref3, ref4, ref5, ref6, shape;
        polygon = new Polygon(vertices);
        polygon.optimize(); // remove duplicate/collinear
        if (polygon.length() < 3) {
      ...
      split_at_sharp_folds(vertices, sharp_angle_rad) {
        var fold, folds, i, k, l, len, n, next_fold, results;
        n = vertices.length;
        folds = (function() {
          var l, ref, results;
          results = [];
        ...
        if (folds.length === 0) {
          ...
          results = [];
          for (k = l = 0, len = folds.length; l < len; k = ++l) {
            fold = folds[k];
  ...
  Blocks = class Blocks {
    ...
    init_physics() {
      var block, ground, l, len, ref, results, user_data;
    ...
    build_mesh(block) {
      var color, geometry, i, indices, l, len, mesh, point, positions, ref, source, texture, uv_scale, uvs;
  ...
  Edges = class Edges {
    ...
    parse() {
      var l, len, len1, m, polygon, ref, results, run, runs, theme, xml_edges, xml_material;
      ...
      results = [];
      for (m = 0, len1 = runs.length; m < len1; m++) {
        run = runs[m];
    ...
    init_graphics() {
      var color, count, geometry, i, l, len, len1, m, polygon, positions, ref, ref1, results, texture, v;
      ref = this.polygons;
      results = [];
      for (l = 0, len = ref.length; l < len; l++) {
        polygon = ref[l];
...
// parse entity xml
  ...
  Limits = class Limits {
    ...
    init_physics() {
      var ground, l, len, ref, results, user_data, vertices, wall;
      ground = Constants.ground;
      ref = this.walls;
      results = [];
      for (l = 0, len = ref.length; l < len; l++) {
  ...
  Sounds = (function() {
    ...
    class Sounds {
      ...
      init() {
        this.engine = new EngineSound(this.level);
  ...
  EngineSound = (function() {
    var LOAD_FALL_TAU, LOAD_RISE_TAU, PITCH_FALL_TAU, PITCH_RISE_TAU;
    ...
    class EngineSound {
      constructor(level) {
        this.level = level;
        this.pitch = 0.0; // smoothed rpm fraction [0..1]
        this.load = 0.0; // smoothed throttle     [0..1]
      }

      init() {
        var c, l, len, now, osc, ref;
        if (!(this.level.options.playable && Constants.engine_sound.enabled)) {
          return;
        }
        if (EngineSound.current) {
          EngineSound.current.destroy();
        }
        EngineSound.current = this;
        this.ctx = PIXI.sound.context.audioContext;
        ...
        this.lowpass.frequency.value = c.lowpass_base;
        this.lowpass.Q.value = 2.0;
        this.lowpass.connect(this.master);
        // soft-clip waveshaper -> lowpass (exhaust growl, driven harder under load)
        this.shaper = this.ctx.createWaveShaper();
        this.shaper.curve = this.distortion_curve();
        ...
        this.osc1 = this.create_oscillator('sawtooth', c.min_frequency, 0.50); // fundamental
        this.osc2 = this.create_oscillator('sawtooth', c.min_frequency * 2.0, 0.22); // 2nd harmonic (detuned)
        this.sub = this.create_oscillator('square', c.min_frequency / 2.0, 0.35); // sub thump
        
        // white noise -> bandpass -> gain -> drive (mechanical/exhaust texture)
        this.noise = this.ctx.createBufferSource();
        this.noise.buffer = this.create_noise_buffer();
        this.noise.loop = true;
        this.noise_filter = this.ctx.createBiquadFilter();
        this.noise_filter.type = 'bandpass';
        this.noise_filter.frequency.value = 400 + c.min_frequency * 6;
        this.noise_filter.Q.value = 0.7;
        this.noise_gain = this.ctx.createGain();
        this.noise_gain.gain.value = c.noise_idle;
        this.noise.connect(this.noise_filter);
        this.noise_filter.connect(this.noise_gain);
        this.noise_gain.connect(this.drive);
        ref = [this.osc1, this.osc2, this.sub, this.noise];
        for (l = 0, len = ref.length; l < len; l++) {
          osc = ref[l];
          osc.start(now);
        }
        this.bind_audio_unlock();
        return this.bind_visibility();
      }
      ...
      create_noise_buffer() {
        var buffer, data, i, l, ref;
        buffer = this.ctx.createBuffer(1, this.ctx.sampleRate, this.ctx.sampleRate); // 1 second
        data = buffer.getChannelData(0);
        for (i = l = 0, ref = data.length; (0 <= ref ? l < ref : l > ref); i = 0 <= ref ? ++l : --l) {
      ...
      bind_audio_unlock() {
        return $(document).on('keydown.engine_sound pointerdown.engine_sound touchstart.engine_sound', () => {
          if (this.ctx && this.ctx.state === 'suspended') {
            return this.ctx.resume();
          } else {
            return $(document).off('.engine_sound');
          }
        });
      }
      ...
      bind_visibility() {
        this.on_visibility = () => {
          if (document.hidden && this.ctx) { // update() restores the volume when back
            this.master.gain.cancelScheduledValues(this.ctx.currentTime);
            return this.master.gain.setTargetAtTime(0.0, this.ctx.currentTime, 0.02);
          }
        };
        return document.addEventListener('visibilitychange', this.on_visibility);
      }

      update() {
        var c, dt, f0, input, moto, now, target_pitch, throttle, volume, wheel_fraction, wobble;
        if (!this.ctx) {
          return;
        }
        now = this.ctx.currentTime;
        dt = this.last_time != null ? Math.min(now - this.last_time, 0.1) : 1.0 / 60.0;
        this.last_time = now;
        c = Constants.engine_sound;
        moto = this.level.moto;
        input = this.level.input;
        throttle = input.up && !moto.dead ? 1.0 : 0.0;
        ...
        if (throttle === 0.0) {
          target_pitch = Math.min(target_pitch, c.coast_max_pitch);
        }
        this.pitch = this.approach(this.pitch, target_pitch, dt, target_pitch > this.pitch ? PITCH_RISE_TAU : PITCH_FALL_TAU);
        this.load = this.approach(this.load, throttle, dt, throttle > this.load ? LOAD_RISE_TAU : LOAD_FALL_TAU);
        f0 = c.min_frequency + Math.pow(this.pitch, c.pitch_curve) * (c.max_frequency - c.min_frequency);
        wobble = (Math.random() - 0.5) * 2.0 * c.idle_wobble * (1.0 - this.pitch); // lumpy idle
        this.osc1.frequency.setTargetAtTime(f0, now, 0.03);
        this.osc2.frequency.setTargetAtTime(f0 * 2.0, now, 0.03);
        this.sub.frequency.setTargetAtTime(f0 / 2.0, now, 0.03);
        this.osc1.detune.setTargetAtTime(wobble, now, 0.05);
        this.osc2.detune.setTargetAtTime(wobble + 8.0, now, 0.05); // constant +8 cents = richer tone
        this.lowpass.frequency.setTargetAtTime(c.lowpass_base + this.pitch * c.lowpass_pitch + this.load * c.lowpass_load, now, 0.05);
        this.noise_filter.frequency.setTargetAtTime(400 + f0 * 6.0, now, 0.05);
        this.noise_gain.gain.setTargetAtTime(c.noise_idle + this.load * c.noise_load + this.pitch * 0.08, now, 0.05);
        this.drive.gain.setTargetAtTime(1.0 + this.load * c.drive_load, now, 0.05);
        if (moto.dead || !c.enabled) {
          return this.master.gain.setTargetAtTime(0.0, now, 0.08); // cut the engine on death
        } else {
          volume = c.volume * (0.28 + 0.22 * this.pitch + 0.50 * this.load);
          return this.master.gain.setTargetAtTime(volume, now, 0.05);
        }
      }
      ...
      destroy() {
        ...
        if (EngineSound.current === this) {
          return EngineSound.current = null;
        }
      }

    };

    EngineSound.current = null; // only one engine can play (levels are never explicitly destroyed)
  ...
  MotoFlipLegacyService = (function() {
    ...
    class MotoFlipLegacyService {
      static run(moto) {
        var all_parts, l, len, len1, m, moto_parts, part, physics_part, results, rider_parts, saved, saved_values;
        ...
        // }
        saved_values = {};
        for (l = 0, len = all_parts.length; l < len; l++) {
          part = all_parts[l];
...
// Reassign correct position + velocity values
        results = [];
        for (m = 0, len1 = all_parts.length; m < len1; m++) {
          part = all_parts[m];
          physics_part = moto_parts.includes(part) ? moto[part] : moto.rider[part];
  ...
  MotoFlipService = (function() {
    ...
    class MotoFlipService {
      static run(moto) {
        var all_parts, body_angle, body_position, l, len, len1, local_offset, m, moto_parts, new_angle, new_offset, new_position, part, physics_part, results, rider_parts, saved, saved_values, world_offset;
        // Accessible through "moto."
        moto_parts = ['body', 'left_wheel', 'right_wheel', 'left_axle', 'right_axle'];
        // Accessible through "moto.rider."
        rider_parts = ['head', 'torso', 'lower_leg', 'upper_leg', 'lower_arm', 'upper_arm'];
  ...
  Assets = class Assets {
    ...
    load(callback) {
      var item, items, l, len, len1, len2, len3, len4, m, o, q, r, ref, ref1, ref2, ref3, ref4, urls;
      items = [];
      ref = this.textures;
      for (l = 0, len = ref.length; l < len; l++) {
        item = ref[l];
      ...
      return PIXI.Assets.load(urls).then(() => {
        var len5, u;
        for (u = 0, len5 = items.length; u < len5; u++) {
          item = items[u];
...
// Sanity-check the decomposition output.
        for (l = 0, len = polygons.length; l < len; l++) {
          polygon = polygons[l];
  ...
  Theme = class Theme {
    ...
    load_theme(xml) {
      var animated, l, len, len1, m, name, texture, xml_sound, xml_sounds, xml_sprite, xml_sprites;
      ...
      for (m = 0, len1 = xml_sounds.length; m < len1; m++) {
