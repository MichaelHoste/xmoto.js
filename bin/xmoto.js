...
(function() {
  var AABB, Assets, BayazitDecomposition, Blocks, Camera, Chain, Circle, Constants, Edges, Entities, Ghost, Ghosts, Infos, Input, Layers, Level, Limits, Listeners, Math2D, Moto, MotoFlipService, Particles, Physics, PhysicsDrawingService, Polygon, PrismaticJoint, Replacements, Replay, ReplayConversionService, RevoluteJoint, Rider, Script, Settings, Sky, Theme, Vec2, World, bind_debug_button, bind_delete_params_buttons, create_form_with_url_params, display_constants, override_constants_by_url_params;
  ...
  Physics = (function() {
    ...
    class Physics {
      ...
      create_rectangles_collisions(position, vertices, name, opts = {}) {
        ...
        for (i = l = 0, len = vertices.length; l < len; i = ++l) {
          ...
          shape = new Polygon([
            ...
            planck.Vec2(v2.x,
            v2.y), // Top-right
            planck.Vec2(v2.x + offsetX,
            v2.y + offsetY), // Bottom-right
            planck.Vec2(v1.x + offsetX,
      ...
      static optimize_vertices(vertices) {
      ...
      static remove_duplicate_vertices(vertices, distance = Settings.linearSlop) {
        var pairs;
      ...
      static remove_collinear_vertices(vertices, angle_rad = 0.01) { // 0.01 = ~0.5°
        var pairs;
        pairs = vertices.map(function(vertex) {
          return [vertex.x, vertex.y];
        });
      ...
      static decompose_to_convex(vertices, strategy = DEFAULT_DECOMPOSE_STRATEGY) {
        ...
        convex_polygons = (function() {
          switch (strategy) {
            case DECOMPOSE_STRATEGIES.QUICK_DECOMP:
              max_level = Math.max(pairs.length, 100);
              return decomp.quickDecomp(pairs, void 0, void 0, void 0, void 0, max_level);
            case DECOMPOSE_STRATEGIES.DECOMP:
              return decomp.decomp(pairs);
            case DECOMPOSE_STRATEGIES.CONVEX_PARTITION:
              polygon = pairs.map(function(pair) {
                return {
                  x: pair[0],
                  y: pair[1]
                };
              });
              return PolyPartition.convexPartition(polygon, true).map(function(convex_polygon) { // true = already CCW, skip its ordering check
                return convex_polygon.map(function(vertex) {
                  return [vertex.x, vertex.y];
                });
              });
            case DECOMPOSE_STRATEGIES.BAYAZIT:
              ...
              degenerate = ((function() {
                var l, len, results;
                results = [];
                for (l = 0, len = partitioned.length; l < len; l++) {
                  polygon = partitioned[l];
                  if (polygon.length < 3) {
              ...
              return partitioned.filter(function(polygon) {
      ...
      static check_intersect_vertices(vertices) {
        var pairs;
        pairs = vertices.map(function(vertex) {
          return [vertex.x, vertex.y];
        });
    ...
    init_physics() {
      ...
      for (l = 0, len = ref.length; l < len; l++) {
        ...
        if (!block.no_collision) {
          ...
          results.push(this.level.physics.create_polygons_collisions(block.position, block.vertices, 'ground', {
