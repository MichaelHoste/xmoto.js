...
(function() {
  var AABB, Assets, BayazitDecomposition, Blocks, Camera, Circle, Constants, Edges, EngineSound, Entities, Ghost, Ghosts, Infos, Input, Layers, Level, Limits, Listeners, Math2D, Moto, MotoFlipLegacyService, MotoFlipService, Particles, Physics, PhysicsDrawingService, Polygon, Replacements, Replay, ReplayConversionService, Rider, Script, Settings, Sky, Sounds, Theme, Vec2, bind_debug_button, bind_delete_params_buttons, create_form_with_url_params, display_constants, override_constants_by_url_params;
  ...
  Camera = class Camera {
    ...
    move() {
      ...
      if (Constants.automatic_scale) {
        ...
        speed = Math2D.distance_between_points({
          x: 0,
          y: 0
        }, velocity);
    ...
    update() {
      var aabb_factor, ctx, cx, cy, delta_x, effective_scale_x, effective_scale_y, half_h, half_w, hh, hw, i, l, len1, level_left, level_top, parallax, ref, scale_layer, size_x, size_y, t, translate_layer, z;
  ...
  Constants = (function() {
    class Constants {
      ...
      static chain_reaction() {
        var element, l, len1, ref, results;
  ...
  Level = class Level {
    constructor(renderer, options) {
      ...
      window.level = this; // to get access through console
      this.show_loading();
      // Context
      this.debug_ctx = $('#xmoto-debug')[0].getContext('2d'); // Debug canvas to validate physics shapes/joints (?debug=true&debug_physics=true)
  ...
  Physics = (function() {
    var CHAIN_SHARP_ANGLE, DEFAULT_FIXTURE, POSITION_ITERATIONS, RECTANGLE_THICKNESS, STEPS_PER_SEC, VELOCITY_ITERATIONS;
    ...
    class Physics {
      constructor(level) {
        ...
        this.physics_drawing_service = new PhysicsDrawingService(this.level.debug_ctx, this.world);
      }

      init() {
        this.last_step_ms = performance.now();
        this.steps = 0;
        return this.step_ms = 1000 / STEPS_PER_SEC; // Time in ms of a single step
      }
      ...
      update() {
        ...
        while (performance.now() - this.last_step_ms > this.step_ms) {
          ...
          this.world.step(1 / STEPS_PER_SEC, VELOCITY_ITERATIONS, POSITION_ITERATIONS);
      ...
      create_polygon_with_rectangles(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        ...
        for (i = l = 0, len1 = vertices.length; l < len1; i = ++l) {
          vertex = vertices[i];
          v1 = vertex;
          v2 = i === vertices.length - 1 ? vertices[0] : vertices[i + 1];
          dx = v2.x - v1.x;
          dy = v2.y - v1.y;
      ...
      create_polygon_with_chains(vertices, position, angle = 0, type = 'static', user_data = {}, opts = {}) {
        ...
        chains = this.split_at_sharp_folds(polygon.vertices, CHAIN_SHARP_ANGLE * Math.PI / 180);
      ...
      split_at_sharp_folds(vertices, sharp_angle_rad) {
        var fold, folds, i, k, l, len1, n, next_fold, results;
        n = vertices.length;
        folds = (function() {
          var l, ref, results;
          results = [];
          for (i = l = 0, ref = n; (0 <= ref ? l < ref : l > ref); i = 0 <= ref ? ++l : --l) {
            if (Physics.sharp_fold(vertices, i, sharp_angle_rad)) {
              results.push(i);
            }
          }
          return results;
        ...
        if (folds.length === 0) {
          ...
          results = [];
          for (k = l = 0, len1 = folds.length; l < len1; k = ++l) {
            fold = folds[k];
            next_fold = folds[(k + 1) % folds.length];
            results.push({
              vertices: Physics.slice_cyclic(vertices, fold, next_fold),
              is_loop: false
            });
          }
          return results;
      ...
      static sharp_fold(vertices, i, sharp_angle_rad) {
        return Math.abs(Physics.turn_angle(vertices, i)) > sharp_angle_rad;
      }

      // Signed angle (radians) between the incoming and outgoing edge at vertices[i]
      static turn_angle(vertices, i) {
        var cross, cur, d_in, d_out, dot, n, next, prev;
        n = vertices.length;
      ...
      static slice_cyclic(vertices, from, to) {
        var l, n, ref, results, step, steps;
        n = vertices.length;
        steps = (to - from + n) % n;
        if (steps === 0) {
          steps = n;
        ...
        results = [];
        for (step = l = 0, ref = steps; (0 <= ref ? l <= ref : l >= ref); step = 0 <= ref ? ++l : --l) {
          results.push(vertices[(from + step) % n]);
        }
        return results;
      }
  ...
  Blocks = class Blocks {
    ...
    parse(xml) {
      var block, l, len1, len2, m, ref, texture_params, vertex, xml_block, xml_blocks, xml_vertex;
      xml_blocks = $(xml).find('block');
      for (l = 0, len1 = xml_blocks.length; l < len1; l++) {
        ...
        block = {
          ...
          usetexture: {
            ...
            color: {
              r: this.parse_color(xml_block, 'r'),
              g: this.parse_color(xml_block, 'g'),
              b: this.parse_color(xml_block, 'b'),
              a: this.parse_color(xml_block, 'a')
            }
          },
          physics: {
        ...
        if (block.vertices.length >= 3) {
          block.edges = new Edges(this.level, block, xml_block);
          block.edges.parse();
    ...
    load_assets() {
      var block, l, len1, len2, m, ref, ref1, results, texture_name;
      ref = this.blocks;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
        block = ref[l];
        ...
        results.push(block.edges.load_assets());
      }
      return results;
    }

    init() {
      var block, l, len1, ref, results;
      this.init_physics();
      this.init_graphics();
      ...
      for (l = 0, len1 = ref.length; l < len1; l++) {
        block = ref[l];
        results.push(block.edges.init());
      }
      return results;
    }

    init_physics() {
      var block, ground, l, len1, ref, results, user_data;
    ...
    init_graphics() {
      var block, l, len1, len2, m, name, now, ref, ref1, results, texture;
      now = performance.now();
      ref = this.blocks;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
    ...
    build_mesh(block) {
      var color, geometry, i, indices, l, len1, mesh, point, positions, ref, source, texture, uv_scale, uvs;
    ...
    init_culling_debug() {
      var block, graphics, l, layer, len1, ref, results;
      if (Constants.debug_culling) {
    ...
    draw_debug_culling() {
      var block, calling_debug, culling_debug, culling_debugs, l, layer, len1, len2, len3, line_width, m, o, ref, ref1, results;
      culling_debugs = Object.values(this.culling_debug_graphics);
      for (l = 0, len1 = culling_debugs.length; l < len1; l++) {
    ...
    visible(block) {
      ...
      if (block.position.islayer && block.position.layerid !== void 0) {
        ...
        return AABB.testOverlap(block.aabb, parallax_layer.camera_aabb);
      } else {
        return AABB.testOverlap(block.aabb, this.level.camera.aabb);
      }
    }
  ...
  Edges = class Edges {
    constructor(level, block, xml_block) {
      ...
      this.materials = []; // Edges could use custom materials (with tint/alpha)
    }

    parse() {
      var l, len1, len2, m, polygon, ref, results, run, runs, theme, xml_edges, xml_material;
      xml_edges = $(this.xml_block).find('edges');
      this.angle = parseFloat(xml_edges.attr('angle')); // Not currently used!
      this.drawmethod = xml_edges.attr('drawmethod'); // Not currently used! Could be: 'angle' | 'in' | 'out'
      ref = xml_edges.find('material');
      for (l = 0, len1 = ref.length; l < len1; l++) {
        xml_material = ref[l];
        this.materials.push({
          ...
          color: {
            r: this.parse_color(xml_material, 'r'),
            g: this.parse_color(xml_material, 'g'),
            b: this.parse_color(xml_material, 'b'),
            a: this.parse_color(xml_material, 'a')
          }
        });
      }
      ...
      results = [];
      for (m = 0, len2 = runs.length; m < len2; m++) {
        run = runs[m];
        theme = this.edge_params(run.texture);
    ...
    build_edge_polygon(vertices, run, theme) {
      var bot_curr, bot_next, depth, dx, dy, i, indices, k, l, lengths, m, n, o, poly_vertices, q, r, ref, ref1, ref2, ref3, ref4, s, scale, span, surface, surface_count, uvs, v;
      n = vertices.length;
      depth = theme.depth;
      scale = theme.scale;
      ...
      for (s = q = 0, ref3 = surface_count; (0 <= ref3 ? q < ref3 : q > ref3); s = 0 <= ref3 ? ++q : --q) {
      ...
      indices = [];
      for (s = r = 0, ref4 = surface_count - 1; (0 <= ref4 ? r < ref4 : r > ref4); s = 0 <= ref4 ? ++r : --r) {
    ...
    load_assets() {
      var l, len1, polygon, ref, results;
      ref = this.polygons;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
        polygon = ref[l];
    ...
    init_graphics() {
      var color, count, geometry, i, l, len1, len2, m, polygon, positions, ref, ref1, results, texture, v;
      ref = this.polygons;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
        polygon = ref[l];
    ...
    update() {
      var block_visible, l, len1, polygon, ref, results;
      if (!Constants.debug_physics) {
    ...
    visible(polygon) {
      ...
      if (this.block.position.islayer && this.block.position.layerid !== void 0) {
        ...
        return AABB.testOverlap(polygon.aabb, parallax_layer.camera_aabb);
      } else {
        return AABB.testOverlap(polygon.aabb, this.level.camera.aabb);
      }
    }
  ...
  Entities = (function() {
    ...
    class Entities {
      ...
      parse(xml) {
        var entity, l, len1, len2, m, name, texture_name, theme_center_x, theme_center_y, theme_height, theme_sprite, theme_width, value, xml_entities, xml_entity, xml_param, xml_params;
        xml_entities = $(xml).find('entity');
// parse entity xml
        for (l = 0, len1 = xml_entities.length; l < len1; l++) {
          ...
          for (m = 0, len2 = xml_params.length; m < len2; m++) {
      ...
      load_assets() {
        var entity, i, l, len1, ref, results;
        ref = this.list;
        results = [];
        for (l = 0, len1 = ref.length; l < len1; l++) {
          entity = ref[l];
      ...
      draw_debug_culling() {
        var entity, l, len1, line_width, ref;
      ...
      compute_aabb(entity) {
        var aabb, bottom, corner, corners, l, left, len1, right, rotated, top, xs, ys;
  ...
  Layers = class Layers {
    ...
    parse(xml) {
      var i, l, layer, len1, xml_layer, xml_layers;
      xml_layers = $(xml).find('layeroffsets layeroffset');
      for (i = l = 0, len1 = xml_layers.length; l < len1; i = ++l) {
    ...
    init_parallax_layers() {
      var l, len1, parallax_data, parallax_layer, ref, results, scale_layer, translate_layer;
      ref = this.list;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
        parallax_data = ref[l];
    ...
    assemble_layers() {
      var i, l, len1, len2, m, parallax, ref, ref1, results;
      ref = this.list;
  ...
  Limits = class Limits {
    ...
    parse(xml) {
      var l, len1, ref, texture, texture_params, wall, xml_limits;
    ...
    load_assets() {
      var l, len1, ref, results, texture_name;
    ...
    init_physics() {
      var ground, l, len1, ref, results, user_data, vertices, wall;
      ground = Constants.ground;
      ref = this.walls;
      results = [];
      for (l = 0, len1 = ref.length; l < len1; l++) {
    ...
    init_graphics() {
      var corner, corners, geometry, i, l, len1, len2, len3, m, name, o, positions, ref, ref1, results, texture, uv_scale, uvs, wall;
      ...
      for (m = 0, len2 = ref1.length; m < len2; m++) {
        ...
        for (i = o = 0, len3 = corners.length; o < len3; i = ++o) {
          corner = corners[i];
    ...
    draw_debug_culling() {
      var l, len1, line_width, ref, wall;
      this.culling_debug.clear();
      ref = this.walls;
      for (l = 0, len1 = ref.length; l < len1; l++) {
        wall = ref[l];
    ...
    visible(wall) {
      return AABB.testOverlap(wall.aabb, this.level.camera.aabb);
    }
  ...
  Replacements = class Replacements {
    ...
    parse(xml) {
      var l, len1, len2, m, new_name, old_name, ref, ref1, results, sound_replacement, sprite_replacement, xml_replacements;
  ...
  Sky = (function() {
    class Sky {
      ...
      load_assets() {
        var l, len1, ref, results, sky_filename;
  ...
  Sounds = (function() {
    ...
    class Sounds {
      constructor(level) {
        this.level = level;
        this.assets = level.assets;
        this.engine = new EngineSound(level);
        this.list = []; // list of loaded sounds
      }

      parse(xml) {
        var l, len1, results, sound_name, sound_names, theme_sound;
        ...
        results = [];
        for (l = 0, len1 = sound_names.length; l < len1; l++) {
  ...
  Ghost = class Ghost {
    ...
    move_with_key_step() {
      var key_step, l, len1, len2, m, part, ref, ref1, results;
  ...
  Ghosts = class Ghosts {
    ...
    load_replays() {
      var l, len1, option_replay, ref, replay, results;
  ...
  Moto = class Moto {
    ...
    init_graphics() {
      var asset_name, axle_name, l, len1, len2, len3, m, o, part, ref, ref1, ref2, wheel_name;
      ref = ['body', 'left_wheel', 'right_wheel', 'left_axle', 'right_axle'];
    ...
    create_body() {
      var position, user_data, vertices;
      position = {
        x: this.player_start.x + this.mirror * Constants.body.position.x,
        y: this.player_start.y + Constants.body.position.y
      };
      vertices = Constants.body.vertices.map((vertex) => {
        return {
          x: this.mirror * vertex.x,
          y: vertex.y
        };
      });
      user_data = {
    ...
    create_axle(part_constants) {
      ...
      vertices = part_constants.vertices.map((vertex) => {
        return {
          x: this.mirror * vertex.x,
          y: vertex.y
        };
    ...
    create_prismatic_joint(axle, part_constants) {
      ...
      joint = new planck.PrismaticJoint(opts, this.body, axle, axle.getWorldCenter(), axis);
      return this.world.createJoint(joint);
    }
    ...
    compute_aabb() {
      ...
      lower1 = this.left_wheel.getFixtureList().getAABB(0).lowerBound;
      lower2 = this.right_wheel.getFixtureList().getAABB(0).lowerBound;
      lower3 = this.rider.head.getFixtureList().getAABB(0).lowerBound;
      // upper position of wheels or head (in case or looping)
      upper1 = this.left_wheel.getFixtureList().getAABB(0).upperBound;
      upper2 = this.right_wheel.getFixtureList().getAABB(0).upperBound;
      upper3 = this.rider.head.getFixtureList().getAABB(0).upperBound;
      aabb = new AABB();
    ...
    visible() {
      return AABB.testOverlap(this.aabb, this.level.camera.aabb);
    }
  ...
  Replay = class Replay {
    ...
    clone() {
      var key, l, len1, len2, m, new_replay, part, ref, ref1, ref2, value;
    ...
    add_key_steps() {
      var key_step, l, len1, len2, m, moto, part, ref, ref1, results, rider;
      moto = this.level.moto;
      rider = moto.rider;
    ...
    last(input) {
      var element, i, input_length, l, last_element, len1, ref, steps;
      last_element = null;
  ...
  Rider = class Rider {
    ...
    init_graphics() {
      var asset_name, l, len1, part, ref, results;
    ...
    create_part(part_constants, name) {
      var angle, position, user_data, vertices;
      position = {
        x: this.player_start.x + this.mirror * part_constants.position.x,
        y: this.player_start.y + part_constants.position.y
      };
      angle = this.mirror * part_constants.angle;
      vertices = part_constants.vertices.map((vertex) => {
        return {
          x: this.mirror * vertex.x,
          y: vertex.y
    ...
    create_neck_joint() {
      ...
      opts = {};
      joint = new planck.RevoluteJoint(opts, this.head, this.torso, axe);
      return this.world.createJoint(joint);
    }
  ...
  EngineSound = (function() {
    class EngineSound {
      ...
      constructor(level) {
        ...
        this.ctx = null;
        this.running = false;
        this.rpm = 0; // smoothed 0..1
        this.throttle = 0; // smoothed 0..1
      }
      ...
      audio_context() {
        return window.__xmoto_audio_ctx != null ? window.__xmoto_audio_ctx : window.__xmoto_audio_ctx = new (window.AudioContext || window.webkitAudioContext)();
      }

      init() {
        var unlock;
        this.ctx = this.audio_context();
        this.build_graph();
        ...
        return document.addEventListener('pointerdown', unlock, {
          once: true
        });
      }

      build_graph() {
        var ctx, gain, l, len1, noise, osc, v, voices;
        ctx = this.ctx;
        ...
        voices = [
          {
            type: 'sine',
            ratio: 0.5,
            gain: 0.32 // sub "thump" (half the fundamental)
          },
          {
            type: 'sawtooth',
            ratio: 1.0,
            gain: 0.50 // fundamental
          },
          {
            type: 'square',
            ratio: 2.0,
            gain: 0.20 // 2nd harmonic
          },
          {
            type: 'sawtooth',
            ratio: 3.0,
            gain: 0.13 // 3rd harmonic
          },
          {
            type: 'sawtooth',
            ratio: 4.0,
            gain: 0.08 // 4th harmonic
          }
        ];
        this.voices = [];
        for (l = 0, len1 = voices.length; l < len1; l++) {
          v = voices[l];
          osc = ctx.createOscillator();
          osc.type = v.type;
          osc.frequency.value = this.idle_hz * v.ratio;
          osc.start();
          gain = ctx.createGain();
        ...
        this.noise_filter = ctx.createBiquadFilter();
        this.noise_filter.type = 'bandpass';
        this.noise_filter.frequency.value = 900;
        this.noise_filter.Q.value = 0.8;
        this.noise_gain = ctx.createGain();
        this.noise_gain.gain.value = 0;
        this.noise_filter.connect(this.noise_gain);
        this.noise_gain.connect(this.master);
        noise = ctx.createBufferSource();
        noise.buffer = this.make_noise_buffer();
        noise.loop = true;
        noise.start();
        return noise.connect(this.noise_filter);
      }

      make_noise_buffer() {
        var buf, ctx, data, i, l, len, ref;
        ctx = this.ctx;
      ...
      update(rpm01, throttle01) {
        ...
        ctx = this.ctx;
        if (!this.running) {
          if (ctx.state === 'suspended') { // still waiting for a user gesture
            return;
          }
          this.running = true;
        }
        // One-pole smoothing of the jittery per-frame inputs
        this.rpm += (rpm01 - this.rpm) * 0.14;
        this.throttle += (throttle01 - this.throttle) * 0.20;
        rpm = this.rpm;
        thr = this.throttle;
        now = ctx.currentTime;
        TWO_PI = 2 * Math.PI;
        ...
        for (l = 0, len1 = ref.length; l < len1; l++) {
          ...
          detune = 1 + voice.detune * (0.35 + idle * 0.55) * Math.sin(now * TWO_PI * voice.beat + voice.phase);
          ...
          g = voice.base_gain * (1 + thr * (voice.ratio - 0.5) * 0.30) * (1 + wobble * voice.ratio * 0.5);
        ...
        volume += chug + rumble;
        volume *= 1 + wobble * 0.6;
        if (this.level.moto.dead) { // engine cuts on crash
          volume = 0;
        }
        return this.master.gain.setTargetAtTime(volume, now, 0.03);
      }
  ...
  MotoFlipLegacyService = (function() {
    var DEBUG;
    ...
    class MotoFlipLegacyService {
      static run(moto) {
        var all_parts, l, len1, len2, m, moto_parts, part, physics_part, results, rider_parts, saved, saved_values;
        // Accessible through "moto."
        moto_parts = ['body', 'left_wheel', 'right_wheel', 'left_axle', 'right_axle'];
        // Accessible through "moto.rider."
        rider_parts = ['head', 'torso', 'lower_leg', 'upper_leg', 'lower_arm', 'upper_arm'];
        ...
        // }
        saved_values = {};
        for (l = 0, len1 = all_parts.length; l < len1; l++) {
          part = all_parts[l];
          physics_part = moto_parts.includes(part) ? moto[part] : moto.rider[part];
          saved_values[part] = {
...
// Reassign correct position + velocity values
        results = [];
        for (m = 0, len2 = all_parts.length; m < len2; m++) {
          part = all_parts[m];
          physics_part = moto_parts.includes(part) ? moto[part] : moto.rider[part];
          ...
          if (DEBUG) {
            results.push(physics_part.setType('static'));
          } else {
            results.push(void 0);
          }
        }
  ...
  MotoFlipService = (function() {
    ...
    class MotoFlipService {
      static run(moto) {
        var all_parts, body_angle, body_position, l, len1, len2, local_offset, m, moto_parts, new_angle, new_offset, new_position, part, physics_part, results, rider_parts, saved, saved_values, world_offset;
        // Accessible through "moto."
        moto_parts = ['body', 'left_wheel', 'right_wheel', 'left_axle', 'right_axle'];
        // Accessible through "moto.rider."
        rider_parts = ['head', 'torso', 'lower_leg', 'upper_leg', 'lower_arm', 'upper_arm'];
        ...
        for (m = 0, len2 = all_parts.length; m < len2; m++) {
          ...
          saved = saved_values[part];
          world_offset = {
            x: saved.position.x - body_position.x,
            y: saved.position.y - body_position.y
          };
          local_offset = Math2D.rotate_point(world_offset, -body_angle, {
            x: 0,
            y: 0
          });
          local_offset.x = -local_offset.x;
          new_offset = Math2D.rotate_point(local_offset, body_angle, {
            x: 0,
            y: 0
          });
          new_position = {
            x: body_position.x + new_offset.x,
            y: body_position.y + new_offset.y
          };
  ...
  PhysicsDrawingService = (function() {
    var BACKGROUND_COLOR, BODY_COLORS, FILL_ALPHA, JOINT_COLOR;
    ...
    class PhysicsDrawingService {
      constructor(ctx, world) {
        this.ctx = ctx;
        this.world = world;
        this.ctx.lineWidth = 0.03; // thickness of draw lines
      }

      draw() {
        var body, joint, results;
        ...
        joint = this.world.getJointList();
        results = [];
        while (joint) {
          this.draw_joint(joint);
          results.push(joint = joint.getNext());
        }
        return results;
      }
      ...
      draw_body(body) {
        ...
        results = [];
        while (fixture) {
          this.draw_shape(fixture.getShape(), body, color);
          results.push(fixture = fixture.getNext());
        }
        return results;
      }

      draw_shape(shape, body, color) {
        var center, edge, vertices;
        switch (shape.getType()) {
          case 'circle':
            ...
            // Circle
            this.ctx.beginPath();
            this.ctx.fillStyle = `rgba(${color},${FILL_ALPHA})`;
            this.ctx.strokeStyle = `rgba(${color},1)`;
            this.ctx.arc(center.x, center.y, shape.m_radius, 0, 2 * Math.PI);
            this.ctx.closePath();
            this.ctx.fill();
            this.ctx.stroke();
          ...
          case 'polygon':
            return this.draw_polyline(shape.m_vertices.map(function(v) {
          ...
          case 'edge':
            ...
            return this.draw_polyline(vertices.map(function(v) {
              return body.getWorldPoint(v);
            }), false, color);
          default:
      ...
      draw_joint(joint) {
        ...
        this.ctx.strokeStyle = `rgba(${JOINT_COLOR},1)`;
        this.ctx.beginPath();
        this.ctx.moveTo(joint.getBodyA().getPosition().x, joint.getBodyA().getPosition().y);
        this.ctx.lineTo(anchor_a.x, anchor_a.y);
        this.ctx.stroke();
        this.ctx.beginPath();
        this.ctx.moveTo(anchor_a.x, anchor_a.y);
        this.ctx.lineTo(anchor_b.x, anchor_b.y);
        this.ctx.stroke();
        this.ctx.beginPath();
        this.ctx.moveTo(anchor_b.x, anchor_b.y);
        this.ctx.lineTo(joint.getBodyB().getPosition().x, joint.getBodyB().getPosition().y);
        return this.ctx.stroke();
      }
  ...
  ReplayConversionService = class ReplayConversionService {
    ...
    static inputs_to_string(inputs) {
      var key, l, len1, len2, m, ref, ref1, step, string;
      string = '';
      ref = ['up_down', 'up_up', 'down_down', 'down_up', 'left_down', 'left_up', 'right_down', 'right_up', 'space_pressed'];
      for (l = 0, len1 = ref.length; l < len1; l++) {
        key = ref[l];
    ...
    static string_to_inputs(string) {
      var i, inputs, key, keys, l, len1, len2, m, name, splitted, step, values;
      inputs = {};
      ...
      for (l = 0, len1 = keys.length; l < len1; l++) {
        ...
        inputs[name] = [];
        if (values[0] !== '') {
          for (i = m = 0, len2 = values.length; m < len2; i = ++m) {
            step = values[i];
    ...
    static key_steps_to_string(key_steps) {
      var a, b, c, d, e, f, key, key_step, l, len1, ref, step, string;
    ...
    static string_to_key_steps(string) {
      var current_interval, element, i, key_step, key_step_string, key_steps, key_steps_string, l, len1, len2, m, part_string, ref, ref1, step_interval, value_string;
      key_steps = {};
      string = LZString.decompressFromBase64(string);
  ...
  Assets = class Assets {
    constructor() {
      ...
      this.sounds = []; // Sound effects needed on every level
      this.moto = []; // moto list
      this.resources = {};
    }
    ...
    load(callback) {
      var item, items, l, len1, len2, len3, len4, len5, m, o, q, r, ref, ref1, ref2, ref3, ref4, urls;
      items = [];
      ref = this.textures;
      for (l = 0, len1 = ref.length; l < len1; l++) {
        item = ref[l];
      ...
      return PIXI.Assets.load(urls).then(() => {
        var len6, u;
        for (u = 0, len6 = items.length; u < len6; u++) {
          item = items[u];
  ...
  BayazitDecomposition = (function() {
    ...
    class BayazitDecomposition {
      ...
      static at(i, vertices) {
        var n;
        n = vertices.length;
        return vertices[((i % n) + n) % n];
      }

      // Vertices from index `i` to `j` (inclusive), walking forward cyclically.
      static copy(i, j, vertices) {
        var k, l, n, ref, ref1, results;
        n = vertices.length;
        while (j < i) {
          j += n;
        }
        results = [];
        for (k = l = ref = i, ref1 = j; (ref <= ref1 ? l <= ref1 : l >= ref1); k = ref <= ref1 ? ++l : --l) {
          results.push(BayazitDecomposition.at(k, vertices));
        }
        return results;
      }
      ...
      static left(a, b, c) {
        return BayazitDecomposition.area(a, b, c) > 0;
      }

      static left_on(a, b, c) {
        return BayazitDecomposition.area(a, b, c) >= 0;
      }
      ...
      static right_on(a, b, c) {
        return BayazitDecomposition.area(a, b, c) <= 0;
      }

      static points_equal(a, b) {
        return a.x === b.x && a.y === b.y;
      }

      // Precondition: CCW polygon
      static is_reflex(i, vertices) {
        var at;
        at = BayazitDecomposition.at;
        return BayazitDecomposition.right(at(i - 1, vertices), at(i, vertices), at(i + 1, vertices));
      }

      static square_dist(a, b) {
        var dx, dy;
        dx = b.x - a.x;
        dy = b.y - a.y;
        return dx * dx + dy * dy;
      ...
      static segments_intersect(a0, a1, b0, b1) {
        var denom, ua, ub;
        if (BayazitDecomposition.points_equal(a0, b0) || BayazitDecomposition.points_equal(a0, b1) || BayazitDecomposition.points_equal(a1, b0) || BayazitDecomposition.points_equal(a1, b1)) {
          return false;
        }
        if (Math.max(a0.x, a1.x) < Math.min(b0.x, b1.x) || Math.max(b0.x, b1.x) < Math.min(a0.x, a1.x)) {
          return false;
        }
        if (Math.max(a0.y, a1.y) < Math.min(b0.y, b1.y) || Math.max(b0.y, b1.y) < Math.min(a0.y, a1.y)) {
          return false;
        }
      ...
      static can_see(i, j, vertices) {
        var at, k, l, ref;
        at = BayazitDecomposition.at;
        if (BayazitDecomposition.is_reflex(i, vertices)) {
          if (BayazitDecomposition.left_on(at(i, vertices), at(i - 1, vertices), at(j, vertices)) && BayazitDecomposition.right_on(at(i, vertices), at(i + 1, vertices), at(j, vertices))) {
            return false;
          }
        } else {
          if (BayazitDecomposition.right_on(at(i, vertices), at(i + 1, vertices), at(j, vertices)) || BayazitDecomposition.left_on(at(i, vertices), at(i - 1, vertices), at(j, vertices))) {
            return false;
          }
        }
        if (BayazitDecomposition.is_reflex(j, vertices)) {
          if (BayazitDecomposition.left_on(at(j, vertices), at(j - 1, vertices), at(i, vertices)) && BayazitDecomposition.right_on(at(j, vertices), at(j + 1, vertices), at(i, vertices))) {
            return false;
          }
        } else {
          if (BayazitDecomposition.right_on(at(j, vertices), at(j + 1, vertices), at(i, vertices)) || BayazitDecomposition.left_on(at(j, vertices), at(j - 1, vertices), at(i, vertices))) {
            return false;
          }
        }
        for (k = l = 0, ref = vertices.length; (0 <= ref ? l < ref : l > ref); k = 0 <= ref ? ++l : --l) {
          if ((k + 1) % vertices.length === i || k === i || (k + 1) % vertices.length === j || k === j) {
            continue;
          }
          if (BayazitDecomposition.segments_intersect(at(i, vertices), at(j, vertices), at(k, vertices), at(k + 1, vertices))) {
            return false;
          }
        }
        return true;
      ...
      static signed_area(vertices) {
        var area, i, l, len1, next, vertex;
        area = 0;
        for (i = l = 0, len1 = vertices.length; l < len1; i = ++l) {
          vertex = vertices[i];
      ...
      static is_ccw(vertices) {
        return vertices.length < 3 || BayazitDecomposition.signed_area(vertices) > 0;
      }

      static force_ccw(vertices) {
        if (!BayazitDecomposition.is_ccw(vertices)) {
          vertices.reverse();
        }
        return vertices;
      }
      ...
      static convex_partition(vertices, splits_left = MAX_SPLITS) {
        var at, best_index, d, highest_score, i, j, l, lower_dist, lower_index, lower_int, lower_poly, m, o, p, ref, ref1, ref2, ref3, score, split_point, upper_dist, upper_index, upper_int, upper_poly;
        if (splits_left <= 0) {
        ...
        for (i = l = 0, ref = vertices.length; (0 <= ref ? l < ref : l > ref); i = 0 <= ref ? ++l : --l) {
          ...
          lower_dist = upper_dist = 2e308;
          lower_index = upper_index = 0;
          lower_int = upper_int = {
            x: 0,
            y: 0
          };
          for (j = m = 0, ref1 = vertices.length; (0 <= ref1 ? m < ref1 : m > ref1); j = 0 <= ref1 ? ++m : --m) {
            // if line i-1,i intersects with edge j-1,j
            if (BayazitDecomposition.left(at(i - 1, vertices), at(i, vertices), at(j, vertices)) && BayazitDecomposition.right_on(at(i - 1, vertices), at(i, vertices), at(j - 1, vertices))) {
            ...
            if (BayazitDecomposition.left(at(i + 1, vertices), at(i, vertices), at(j + 1, vertices)) && BayazitDecomposition.right_on(at(i + 1, vertices), at(i, vertices), at(j, vertices))) {
              p = BayazitDecomposition.line_intersect(at(i + 1, vertices), at(i, vertices), at(j, vertices), at(j + 1, vertices));
          ...
          if (lower_index === (upper_index + 1) % vertices.length) {
            ...
            while (upper_index < lower_index) {
              upper_index += vertices.length;
            }
            for (j = o = ref2 = lower_index, ref3 = upper_index; (ref2 <= ref3 ? o <= ref3 : o >= ref3); j = ref2 <= ref3 ? ++o : --o) {
              if (BayazitDecomposition.can_see(i, j, vertices)) {
                score = 1 / (BayazitDecomposition.square_dist(at(i, vertices), at(j, vertices)) + 1);
                if (BayazitDecomposition.is_reflex(j, vertices)) {
                  if (BayazitDecomposition.right_on(at(j - 1, vertices), at(j, vertices), at(i, vertices)) && BayazitDecomposition.left_on(at(j + 1, vertices), at(j, vertices), at(i, vertices))) {
                    score += 3;
                  } else {
                    score += 2;
                  }
        ...
        return [vertices]; // already convex
      }
      ...
      static find_self_intersection(vertices) {
        var a0, a1, b0, b1, i, j, l, m, n, ref, ref1, ref2;
        n = vertices.length;
        for (i = l = 0, ref = n; (0 <= ref ? l < ref : l > ref); i = 0 <= ref ? ++l : --l) {
          for (j = m = ref1 = i + 1, ref2 = n; (ref1 <= ref2 ? m < ref2 : m > ref2); j = ref1 <= ref2 ? ++m : --m) {
            if (j === i + 1 || (i === 0 && j === n - 1)) { // edges sharing a vertex aren't a crossing
              continue;
            }
            a0 = vertices[i];
            a1 = vertices[(i + 1) % n];
            ...
            if (BayazitDecomposition.segments_intersect(a0, a1, b0, b1)) {
              return {
                i: i,
                j: j,
                point: BayazitDecomposition.line_intersect(a0, a1, b0, b1)
              };
            }
          }
        }
      ...
      static split_at_intersection(vertices, i, j, point) {
        return [[point].concat(BayazitDecomposition.copy(i + 1, j, vertices)), [point].concat(BayazitDecomposition.copy(j + 1, i, vertices))];
      }
      ...
      static resolve_self_intersections(vertices, splits_left = MAX_SPLITS) {
        var crossing, loop1, loop2;
        if (splits_left <= 0) {
          return [vertices];
        }
        ...
        return BayazitDecomposition.resolve_self_intersections(loop1, splits_left - 1).concat(BayazitDecomposition.resolve_self_intersections(loop2, splits_left - 1));
      }
      ...
      static decompose(vertices) {
        var simple_loops;
        simple_loops = BayazitDecomposition.resolve_self_intersections(vertices);
        return simple_loops.reduce((function(all, simple_loop) {
          return all.concat(BayazitDecomposition.convex_partition(simple_loop));
        }), []);
      }
  ...
  override_constants_by_url_params = function(params) {
    var array, array_key, array_keys, i, key, l, len1, value;
    for (key in params) {
      value = params[key];
      array_keys = key.split('.');
      array = Constants;
      for (i = l = 0, len1 = array_keys.length; l < len1; i = ++l) {
  ...
  display_constants = function() {
    var html, key, l, len1, ref, ref1, sub_key, sub_value, value;
  ...
  Polygon = (function() {
    var COLLINEAR_MAX_ANGLE_RAD, DECOMPOSE_STRATEGIES, DEFAULT_DECOMPOSE_STRATEGY, DUPLICATE_MAX_DISTANCE, SPLIT_MAX_VERTICES;

    class Polygon {
      // array of {x, y:}
      constructor(vertices) {
        this.vertices = Array.from(vertices); // Make copy
      ...
      is_convex(epsilon = 1e-9) {
        var a, b, c, cross, current_sign, i, l, ref, sign;
        if (this.vertices.length < 3) {
          return false;
        }
        sign = 0;
        for (i = l = 0, ref = this.vertices.length; (0 <= ref ? l < ref : l > ref); i = 0 <= ref ? ++l : --l) {
          ...
          c = this.vertices[(i + 2) % this.vertices.length];
      ...
      is_concave(epsilon = 1e-9) {
        return !this.is_convex(epsilon);
      }

      // Simple polygons don't intersect themselves
      is_simple() {
        var pairs;
        pairs = this.vertices.map(function(vertex) {
          return [vertex.x, vertex.y];
        });
        return decomp.isSimple(pairs);
      }
      ...
      has_duplicate_vertices(distance = DUPLICATE_MAX_DISTANCE) {
        ...
        return this.vertices.length !== pairs.length;
      }
      ...
      optimize() {
        ...
        return this.remove_collinear_vertices;
      }
      ...
      remove_duplicate_vertices(distance = DUPLICATE_MAX_DISTANCE) {
        ...
        decomp.removeDuplicatePoints(pairs, distance);
        if (this.vertices.length === pairs.length) {

        // Do nothing if the number if the same
        } else if (pair.length >= 3) {
          console.warn(`XMoto warning: ${this.vertices.length - pairs.length} duplicate vertices have been removed.`);
          return this.vertices = pairs.map(function(pair) {
            return {
              x: pair[0],
              y: pair[1]
            };
          ...
          console.error(`XMoto error: polygon degenerated from ${this.vertices.length} to ${pairs.length} vertex(es) after removing duplicates, and was ignored.`);
      ...
      remove_collinear_vertices(angle_rad = COLLINEAR_MAX_ANGLE_RAD) {
        ...
        // Do nothing if the number if the same
          ...
          console.warn(`XMoto warning: ${this.vertices.length - pairs.length} collinear vertices have been removed.`);
          return this.vertices = pairs.map(function(pair) {
            return {
              x: pair[0],
              y: pair[1]
          ...
          console.error(`XMoto error: polygon degenerated from ${this.vertices.length} to ${pairs.length} vertex(es) after removing collinear, and was ignored.`);
          return this.vertices = [];
        }
      }
      ...
      split(max_vertices = SPLIT_MAX_VERTICES) {
        var left, left_polygons, mid, n, right, right_polygons;
        ...
        left = this.vertices.slice(0, mid + 1);
        right = this.vertices.slice(mid).concat([this.vertices[0]]);
        left_polygons = new Polygon(left).split(max_vertices);
        right_polygons = new Polygon(right).split(max_vertices);
        return left_polygons.concat(right_polygons);
      }
      ...
      decompose(strategy = DEFAULT_DECOMPOSE_STRATEGY) {
        var l, len1, polygon, polygons;
        if (!_.values(DECOMPOSE_STRATEGIES).includes(strategy)) {
          throw new Error(`XMoto error: unknown Polygon#decompose strategy '${strategy}'`);
        }
        
        // Force Bayazit strategy if self-intersecting polygon (the only one that supports it)
        if (this.self_intersect() && strategy !== DECOMPOSE_STRATEGIES.BAYAZIT) {
          console.warn(`XMoto warning: polygon intersects itself, can't be split with \"${strategy}\" strategy into convex polygons.`);
          return [new Polygon(this.vertices)];
        }
        polygons = (function() {
          switch (strategy) {
            case DECOMPOSE_STRATEGIES.POLY_QUICK_DECOMP:
              return this._poly_quick_decomp_decomposition();
            case DECOMPOSE_STRATEGIES.POLY_DECOMP:
              return this._poly_decomp_decomposition();
            case DECOMPOSE_STRATEGIES.POLY_PARTITION:
              return this._poly_partition_decomposition();
            case DECOMPOSE_STRATEGIES.BAYAZIT:
              return this._bayazit_decomposition();
          }
        }).call(this);
// Sanity-check the decomposition output.
      ...
      _poly_quick_decomp_decomposition() {
        ...
        polygons = decomp.quickDecomp(pairs, void 0, void 0, void 0, void 0, max_level);
        return polygons.map(function(vertices) {
      ...
      _poly_partition_decomposition() {
        ...
        decomp.makeCCW(pairs);
        this.vertices = pairs.map(function(pair) {
          return {
            x: pair[0],
            y: pair[1]
          };
        });
        polygons = PolyPartition.convexPartition(this.vertices, true);
        return polygons.map(function(vertices) {
          return new Polygon(vertices);
        });
      }
      ...
      _bayazit_decomposition() {
        ...
        polygons = BayazitDecomposition.decompose(this.vertices);
        return polygons.map(function(vertices) {
          return new Polygon(vertices);
        });
      }
  ...
  Theme = class Theme {
    ...
    load_theme(xml) {
      var animated, l, len1, len2, m, name, texture, xml_sound, xml_sounds, xml_sprite, xml_sprites;
      xml_sprites = $(xml).find('sprite');
      for (l = 0, len1 = xml_sprites.length; l < len1; l++) {
      ...
      for (m = 0, len2 = xml_sounds.length; m < len2; m++) {
        xml_sound = xml_sounds[m];
    ...
    sound_params(name) {
      return this.sounds[name.toLowerCase()];
    }
