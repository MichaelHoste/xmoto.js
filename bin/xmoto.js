...
// Reassign correct position + velocity values
  ...
  MotoFlipService = (function() {
    ...
    class MotoFlipService {
      static run(moto) {
        var all_parts, body_pos, l, len, len1, local_offset, m, moto_parts, new_angle, new_offset, new_position, old_angle, old_angular, part, physics_part, results, rider_parts, saved, saved_body, saved_values, world_offset;
        // Accessible through "moto."
        moto_parts = ['body', 'left_wheel', 'right_wheel', 'left_axle', 'right_axle'];
        // Accessible through "moto.rider."
        rider_parts = ['head', 'torso', 'lower_leg', 'upper_leg', 'lower_arm', 'upper_arm'];
...
// the now-mirrored body).
        ...
        for (m = 0, len1 = all_parts.length; m < len1; m++) {
          ...
          saved = saved_values[part];
          world_offset = {
            x: saved.position.x - body_pos.x,
            y: saved.position.y - body_pos.y
          };
          local_offset = Math2D.rotate_point(world_offset, -old_angle, {
            x: 0,
            y: 0
          });
          local_offset.x = -local_offset.x;
          new_offset = Math2D.rotate_point(local_offset, old_angle, {
            x: 0,
            y: 0
          });
          new_position = {
            x: body_pos.x + new_offset.x,
            y: body_pos.y + new_offset.y
          };
